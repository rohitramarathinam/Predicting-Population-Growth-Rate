''' This program models ridge regression, experiments with feature scaling, selection, and hyperparameter tuning, and evaluates the performance'''
...
def model(X_train, X_test, y_train, y_test):
    rr = Ridge(alpha=0.1) # **best_params_ridge
...
def hyper_tune(X_train, X_test, y_train, y_test):
    # create grid of parameters to hypertune
    param_grid = {'alpha': [0.1, 1, 10]} # [float(0.1), float(1.0), float(10.0)]

    # create regressor
    _rr = Ridge()

    # perform grid search and obtain best hyperparameters
    grid_search_ridge = GridSearchCV(_rr, param_grid, cv=5, scoring='neg_mean_squared_error')
